...
Written 2026-09-17, restructured 2026-09-18 after review. Implemented in `organization_seed.json`
(the data) and `org_code_families.json` (how a parser recognizes office codes). This document is
...
| `notes`, `review_status`, `reviewed_by` | free text; `draft` until a human reviews; reviewer record or null |
...
| --- | --- |
| `id` | `obs:NNN` |
| `source_url`, `source_revision` | the document and the exact copy (Wayback capture timestamp or retrieval date, plus the hash prefix from `documents_manifest.jsonl`) |
...
| `observation_ids`, `evidence_class` | what the claim rests on; `directly_documented` or `inferred` |
...
| `drafted_by`, `reviewed_by`, `reviewed_on` | the assistant's draft record, and the human reviewer's identity and date (null until filled) |
...
table or the ancestry walk unless a reviewer promotes them to relationships with new observations.
...
2. Normalize (upper-case, strip a ` - NAME` suffix, collapse spaces and hyphens) and look the
   ...
   `org_code_families.json`; families say what kind of organization a code denotes, so an NIWC
...
- A wrong claim is never closed with an end date. It gets `review_status: retracted`, a
  ...
  interpretation. The record stays in the file. Three retractions exist today: the two PMS 485
  parent relationships whose effective dates were invented (`rel:059`, `rel:060`, replaced by
  `rel:061`, `rel:062` with dates unknown) and PMW 250's parentage, which was an inference and now
  lives in `int:003`.
- Reviewer identity and date are recorded in `reviewed_by` / `reviewed_on`, separately from the
  assistant's `drafted_by`. Everything is `draft` until a human fills them.
...
What remains unknown: whether PEO C4I persists as an organization inside the PAE, and which
...
observations are evidence references (the proposed evidence layer) pointing at
`documents_manifest.jsonl` rows. Fields with no column yet: `effective_dates_status`,
`current_status`, `scope_as_stated`, `review_status` with `retraction`, the reviewer record, and
interpretations as a table of their own. `consolidated_into` is a new relationship type.
